    ...
    return SafeArea(
      child: BlocProvider(
        create: (context) => getIt<CategoryByIdCubit>()
          ..getBannerCategoriesById(id!)
          ..getProductCategoriesById(id!)
          ..getBrandCategoriesById(id!),
        child: BlocBuilder<CategoryByIdCubit, CategoriesByIdState>(
          builder: (context, state) {
            if (state is BrandCategoriesByIdSuccess ||
                state is ProductCategoriesByIdSuccess ||
                state is BannerCategoriesByIdSuccess ||
                state is ChangeSelectedIndexBrandCategoryState) {
              return Scaffold(
                  ...
                  slivers: [
                    SliverAppBar(
                      ...
                      flexibleSpace: FlexibleSpaceBar(
                        background:
                            // Image.asset(AppImages.nike, fit: BoxFit.cover),
                            Image.network(

                          "https://ltfpjeeclvrtomahvqyd.supabase.co/storage/v1/object/public/${CategoryByIdCubit.get(context).banner?.imageUrl}",
                          fit: BoxFit.fill,
                        ),
                      ),
                    ),
                    ...
                    SliverToBoxAdapter(
                      child: SizedBox(
                        ...
                        child: ListView.builder(
                            ...
                            itemCount:
                                CategoryByIdCubit.get(context).brands.length,
                            itemBuilder: (context, index) {
                              return InkWell(
                                onTap: () {
                                  CategoryByIdCubit.get(context)
                                      .changeSelectedIndexBrandCategory(index);
                                },
                                child: Padding(
                                  padding:
                                  ...
                                  child: Column(
                                    children: [
                                      CircleAvatar(
                                        radius: 35.w,
                                        backgroundColor: CategoryByIdCubit.get(
                                                        context)
                                                    .selectedIndexBrandCategory ==
                                                index
                                            ? AppColors.primaryColor
                                            : AppColors.lightGreyColor,
                                        child: Image.network(
                                          fit: BoxFit.fill,
                                          "https://ltfpjeeclvrtomahvqyd.supabase.co/storage/v1/object/public/${CategoryByIdCubit.get(context).brands[index].imageUrl}",
                                        ),
                                      ),
                                      SizedBox(
                                        height: 5.h,
            ...
            } else if (state is CategoriesByIdInitial ||
                state is CategoriesByIdLoading) {
              return Scaffold(
                body: CustomScrollView(
                  slivers: [
                    SliverAppBar(
